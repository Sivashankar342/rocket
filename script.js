...
document.getElementById("gpsTracker").addEventListener("click", function () {
  showLocation();
});
...
function login() {
  ...
  if (username === "nithish kumar" && password === "siva") {
    // Hide login container and show weather container
...
function getWeather() {
  ...
  // Use the city from the dropdown if it's selected, otherwise use the typed city
  const city = cityDropdown || cityInput;

  if (!city) {
    document.getElementById(
      "weatherResult"
    ).innerHTML = `<p>Please enter or select a city.</p>`;
    return;
  }
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      if (data.cod === 200) {
        ...
        // Check if the city is Chennai and add guidelines and safe places
...
function showLocation() {
  // Display the VIT Vellore location iframe.
